from cv2 import getRectSubPix, xfeatures2d, BFMatcher, drawMatchesKnn, imwrite
        ...
        for x in range(min_x, stitched_image_w):
            if y+offset_y < stitched_image_h and x+offset_x < stitched_image_w:
                ...
                if 0 < xp_stitched < image2_w and 0 < yp_stitched < image2_h:
                    pixel = getRectSubPix(image2, (1, 1), (xp_stitched, yp_stitched))
...
def stitch_two_images(image1, image2, siftDisplayName, saveSiftMatches, siftFilePath,
    ...
    if saveSiftMatches:
        sift_matches = drawMatchesKnn(image1, kp1, image2, kp2, matches1to2=good_matches, outImg=None,
                                      flags=DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        imwrite(siftFilePath, sift_matches)
    # End source
