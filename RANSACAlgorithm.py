import numpy as np
from cv2 import perspectiveTransform, findHomography, vconcat, drawMatchesKnn, imwrite
import random
import warnings
...
def project(x1, y1, H):
    ...
    # hom_test = perspectiveTransform(a, H)
...
def RANSAC(matches, kp1, kp2, kp_matches, numIterations, inlierThreshold, image1, image2, displayName, save_ransac_matches, ransacFilePath):
    ...
    for i in range(0, numIterations):
        ...
        H, status = findHomography(inlier_src_pts, inlier_dst_pts, 0)
    ...
    hom, status = findHomography(inlier_src_pts, inlier_dst_pts)
    homInv = np.linalg.inv(hom)     # Inverse of hom

    # Display inlier matches
    matching_image = vconcat(image1, image2)
    matching_image = drawMatchesKnn(image1, kp1, image2, kp2, matches1to2=inlier_matches, outImg=matching_image, flags=2)
    if save_ransac_matches:
        print("RANSAC Matches saved to " + ransacFilePath)
        imwrite(ransacFilePath, matching_image)
