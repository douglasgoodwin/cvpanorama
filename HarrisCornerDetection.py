...
def calculate_harris_corner_response(Ix, Iy, sigma):
    ...
    Ixx = GaussianBlur(Ixx, (sigma, sigma), 1, 1)
    Iyy = GaussianBlur(Iyy, (sigma, sigma), 1, 1)
    Ixy = GaussianBlur(Ixy, (sigma, sigma), 1, 1)
    ...
    for y in range(original_image.shape[1]):
        for x in range(original_image.shape[0]):
            if (int(harris_response[x, y]) > threshold) and local_maximum_neighborhood(harris_response, x, y):
                ...
                circle(harris_corners, (y, x), 5, (0, 0, 255), 1)
...
def find_interest_points(src, thresh_percent, saveCornerImage, savedImagePath):
    ...
    grayscale_img = np.float32(cvtColor(src, COLOR_BGR2GRAY))
    ...
    if saveCornerImage:
        ...
        imwrite(savedImagePath, harris_corners)

    #  KeyPoint (float x, float y, float _size, float _angle=-1, float _response=0, int _octave=0, int _class_id=-1)
    # for point in interest_points:
    #     key_points.append(KeyPoint(point[0], point[1], _size=3))
    #
    # return key_points
